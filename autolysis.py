...
def describe_image_with_gpt4o_mini(image_path, column_info):
    """
    Uses the GPT-4o-mini Vision API to describe an image.

    Parameters:
    image_path (str): Path to the image file.
    column_info (str): Information about the data columns to include in the prompt.

    Returns:
    str: Description of the image generated by GPT-4o-mini Vision API.
    """
    api_url = "http://aiproxy.sanand.workers.dev/openai/v1/chat/completions"
    api_key = token  # Token for AI Proxy
    ...
    prompt = (
        f"Describe the following scatter plot image with detail:low. The plot is based on the dataset "
        f"with the following column information: {column_info}. Key patterns, trends, or anomalies "
        f"should be highlighted.\n\n"
    )
    ...
    payload = {
        "model": "gpt-4o-mini",
        "messages": [
            {"role": "user", "content": [prompt, {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{base64_image}", "detail": "low"},}]}
        ]
    }
...
def generate_markdown_summary(input_file, data, key_stats, significant_corr, outliers, col1, col2, reg_result, img_desc):
    """
...
3. Discuss the **outliers** detected using the Z-score method:
    {outliers}
4. Describe the **scatter plot** visualized between {col1} and {col2}, stored as "scatter_plot.png". Explain the relationship between the variables and any key trends or anomalies visible in the plot from the following description
{img_desc}
Also, include the image in the markdown file.
...
- Try Using the result from linear regression to make a prediction.
...
def main():
    ...
    # Get the description of scatter plot plotted
    column_info = "Columns: [{col1}, {col2}]."
    img_description = describe_image_with_gpt4o_mini(output_dir + "scatter_plot.png", column_info)
    ...
    content = generate_markdown_summary(input_file, csv_data, key_stats, significant_corr, outliers, col1, col2, reg_result, img_description)
